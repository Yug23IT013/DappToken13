...
contract('DappToken13', function(accounts) {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', function() {
    return DappToken13.deployed().then(function(instance) {
      ...
      assert.equal(name, 'DappToken13', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'DAPP13', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'DappToken13 v1.0', 'has the correct standard');
    });
  });
